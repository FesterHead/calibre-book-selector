...
5. **NO Unversioned Changes:** Never update `__init__.py` without updating `CHANGELOG.md`, or vice versa. Versions must match SemVer format exactly.
...
5. **Version Synchronization:** Ensure `version = (X, Y, Z)` in `__init__.py` and `[X.Y.Z]` in `CHANGELOG.md` are always kept identical.
